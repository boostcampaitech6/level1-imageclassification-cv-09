...
주최 측에서 제공되었던 베이스라인 코드를 활용하여 모델을 돌려본 결과 여러 문제점을 확인하고 이를 해결하기 위한 방법을 적용했다. 첫째, 코드의 복잡성과 유지 보수 문제를 해결하기 위해 모델, 손실 함수, 옵티마이저 등을 별도의 파일로 분리하고, 필요한 컴포넌트를 train.py에서 동적으로 불러올 수 있게 했다. 둘째, 40개가 넘는 하이퍼 파라미터 관리의 어려움을 개선하기 위해 인자 파싱 방식에서 설정 파일 기반의 YAML 파일 방식으로 변경했다. 셋째, 모델의 가중치 관리 문제를 해결하기 위해 실험마다 고유한 시리얼을 생성하고, 최적의 메트릭을 달성할 때만 가중치를 저장하는 방식을 채택하고, Early Stopping을 구현하여 과적합을 방지했다.
...
<div align="center" >
 ...
 <img width="600" alt="image" src="https://github.com/boostcampaitech6/level1-imageclassification-cv-09/assets/46400961/20a6dcbd-adf9-42aa-9f4f-97930f3e9b48">
...
<div align="center" >
 ...
 <img width="400" alt="image" src="https://github.com/boostcampaitech6/level1-imageclassification-cv-09/assets/46400961/14b95703-4c2f-4a3a-bcc3-0a024709aaf3">
 


</div>

멀티 클래스 학습은 위에서 소개한 18개 클래스를 단일 태스크로 처리하지만 멀티태스크 학습은 하드 파라미터 공유와 소프트 파라미터 공유, 두 가지 접근 방식을 통해 문제를 다룬다. 비교 실험을 진행한 결과, 멀티 클래스 학습과 하드 파라미터 공유를 사용한 멀티태스크 학습은 F1-score에서 비슷한 성능을 보였으나, 멀티 클래스 학습이 정확도에서 미세하게 더 우수했다. 반면에 소프트 파라미터 공유 방식을 적용한 멀티태스크 학습은 F1-score에서도 안정적인 성능을 유지하면서 정확도 면에서 가장 높은 결과를 달성했다.
...
<div align="center" >
  <img width="800" alt="image" src="https://github.com/boostcampaitech6/level1-imageclassification-cv-09/assets/46400961/e785eaeb-cf53-4984-a194-608778f67951">
